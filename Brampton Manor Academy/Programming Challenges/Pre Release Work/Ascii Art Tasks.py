...
def HorizontalFlip(Grid, Header):
    ...
    for ThisRow in range(Header.Height):
        ...
        for ThisColumn in reversed(range(Header.Width)):
            DataStructure.append(Grid[ThisRow][ThisColumn])
            # print(Grid[ThisRow][ThisColumn], end='')
        print(''.join(DataStructure))
...
def FindSecretChar(PixelValue, Key):
    ...
    result = PixelValue - int(Key)
    character = "_"
    if result == 0:
        character = " "
    elif result > 0 and result <= 26:
        character = alphabet[result - 1]
    return character
    ...
    try:
        ...
        for Row in range(Header.Height):
            for Column in range(Header.Width):
                ...
                hidden_message += FindSecretChar(PixelValue, Key)
