...
def ResetDataStructures():
  ...
  :description: Creates an empty 10 by 10 2D array (1st position accessed by 0, 10th by 9)
...
def LoadPuzzleFile(PuzzleName, Puzzle):
  ...
  :description: Loads a puzzle and checks if file is empty, or doesnt exist. Reads the file ignoring the last letter (\n)
...
def LoadSolution(PuzzleName, Solution):
  ...
  :parameters: String, Array
  :Description: Checks if solution file exists and fills solution with data from solution file and returns it
  :return type: Array, Boolean
  """
  OK = True
  try:
...
def ResetAnswer(PuzzleName, Answer):
  ...
  :parameters: String, Array
  :Description: Resets the board to what it originally was
  :return type: Array
  """
  Answer[0] = PuzzleName
  Answer[1] = "0"
...
def TransferPuzzleIntoGrid(PuzzleName, PuzzleGrid, Puzzle, Answer):
  ...
  :Description: transfers data from puzzle into puzzlegrid array
...
def LoadPuzzle(PuzzleGrid, Puzzle, Answer, Solution):
  ...
  :description: Loads puzzle based on user input and transfers the file contents into puzzlegrid
  :return type: Array, Array, Array, Array
...
def TransferAnswerIntoGrid(PuzzleGrid, Answer):
  ...
  :description: Transfers user input into the grid
  :return type: Array
  """
  for Line in range(3, int(Answer[2]) + 3):
    ...
    Column = int(CellInfo[1])
    Digit = CellInfo[2]
    # Test = str(Row) + str(Column) + str(Digit)
    # FilOut = open(f"{Answer[0]}P.txt", 'r')
    # if int(Test) not in FilOut:
    PuzzleGrid[Row][Column] = Digit
  return PuzzleGrid
...
def LoadPartSolvedPuzzle(PuzzleGrid, Puzzle, Answer, Solution):
  ...
  :parameters: Array Array Array Array
  :description: Opens a partially solved puzzle and checks if it exists/is corrupt
  :return type: Array Array Array Array
...
def DisplayGrid(PuzzleGrid):
  ...
  :parameters: Array
  :description: Prints the grid onto the screen
...
def SolvePuzzle(PuzzleGrid, Puzzle, Answer):
  ...
  :description: Stores the users inputs and then adds it onto the grid
  ...
  if PuzzleGrid[0][0] != 'X':
    ...
    while CellInfo != EMPTY_STRING:
      ...
      if InputError:
        ...
        InputError = False
        x = [i[0:2] for i in Puzzle if i != '']

        if (str(Row) + str(Column)) in x:
          InputError = True
          print("You may not overwrite these numbers! \n")

        if checkIfEntered(PuzzleGrid, Row, Column, Digit):
          InputError = True
          print("You may not use duplicates in the same row, column or boxes \n")
...
def checkIfEntered(PuzzleGrid, Row, Column, Digit):
  ...
  for i in range(1,GRID_SIZE+1):
    ...
    if PuzzleGrid[Row][i] == Digit or PuzzleGrid[i][Column] == Digit:
      Error = True
  ...
  Row = 3*(Row // 3) + 1 if Row % 3 != 0 else (Row // 3)
  Column = 3*(Column // 3) + 1 if Column % 3 != 0 else (Column // 3)
...
def DisplayMenu():
  ...
  :Description: Outputs input options
  """
  print()
  print("Main Menu")
  print("=========")
...
def GetMenuOption():
  """
  :description: returns users choice
  :return type: Char
  """
  Choice = EMPTY_STRING
  while len(Choice) != 1:
...
def KeepPuzzle(PuzzleGrid, Answer):
  ...
  :parameters: Array Array
  :description: Saves users file as a partially solved file according to their choice
...
def CheckSolution(PuzzleGrid, Answer, Solution):
  ...
  :parameters: Array Array Array
  :description: Checks if users solution is correct and adds/minuses errorcount accordingly
...
def DisplayResults(Answer):
  ...
  :parameters: Array
  :description: Displays users score and progress
  """
  if int(Answer[2]) > 0:
...
def NumberPuzzle():
  ...
  :description: Chooses a path based on user input
