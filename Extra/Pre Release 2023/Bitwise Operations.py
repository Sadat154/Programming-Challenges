...
def ExtractOpCode(Instruction, LineNumber, Memory):
    if len(Instruction) > 9:
        OpCodeValues = ["LDA", "STA", "LDA#", "HLT", "ADD", "JMP", "SUB", "AND", "ORR", "EOR", "MVN", "CMP#", "BEQ", "SKP", "JSR", "RTN", "   "]
...
def ExecuteAND(Memory, Registers, Address): # Format = AND VAR, VAR in decimal form
    Registers[ACC] = BitwiseOperations(Registers[ACC], Memory[Address].OperandValue, "AND")
...
def ExecuteORR(Memory, Registers, Address):
    Registers[ACC] = BitwiseOperations(Registers[ACC], Memory[Address].OperandValue, "ORR")
...
def ExecuteEOR(Memory, Registers, Address):
    Registers[ACC] = BitwiseOperations(Registers[ACC], Memory[Address].OperandValue, "EOR")
...
def ExecuteMVN(Memory, Registers, Address):
    Registers[ACC] = BitwiseOperations(Registers[ACC], Memory[Address].OperandValue, "MVN")
...
def BitwiseOperations(Value, Operand, Operation):
    AND_Dict = {"11": "1",
                "10": "0",
                "01": "0",
                "00": "0",
    ...
    for i in range(len(BinaryValue)):
        Combined = BinaryValue[i] + BinaryOperand[i]

        if Operation == "AND":
            FinalBinary += AND_Dict[Combined]
        elif Operation == "EOR":
            FinalBinary += EOR_Dict[Combined]
        elif Operation == "ORR":
            FinalBinary += ORR_Dict[Combined]
        elif Operation == "MVN":
            FinalBinary += MVN_Dict[BinaryValue[i]]


    return ConvertToDecimal(FinalBinary)
