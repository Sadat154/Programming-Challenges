...
def ExtractOpCode(Instruction, LineNumber, Memory):
    if len(Instruction) > 9:
        OpCodeValues = ["LDA", "STA", "LDA#", "HLT", "ADD", "ADD#", "SUB#", "CMP", "JMP", "SUB", "CMP#", "BEQ", "SKP", "JSR", "RTN", "   "]
...
def Execute(SourceCode, Memory):
    ...
    while OpCode != "HLT":
        ...
        if OpCode == "LDA":
            ...
            Registers = ExecuteADDimm(Memory, Operand)
        elif OpCode == "JMP":
            ...
            Registers == ExecuteCMP(Registers, Memory, Operand)
        elif OpCode == "BEQ":
            Registers = ExecuteBEQ(Registers, Operand)
        elif OpCode == "SUB":
            Registers = ExecuteSUB(Memory, Registers, Operand)
        elif OpCode == "SUB#":
            Registers = ExecuteSUBimm(Memory, Operand)
        elif OpCode == "SKP":
            ExecuteSKP()
        elif OpCode == "RTN":
